...
Minimal end-to-end workflow: **CSV → PKL → finetune**.
...
- Put your CSV under `datasets/` (or anywhere you like).
- Ensure it contains at least `SMILES0` and your label column (e.g., `Tg`).

### 3) Preprocess CSV → PKL
...
Main output used for training:
- `data/processed/your_dataset_name/main/split.pkl`

### 4) Finetune (single GPU)
...
The pretrain weights involved in this work can be found in 10.5281/zenodo.18509440, and the github respoitory of Uni-Mol2 for model capacity study.
